...
import TextAreaSheet from '../../../ui/composites/TextAreaSheet';
import TokenStrengthSheet from '../../../ui/composites/TokenStrengthSheet';
import { IconEdit, IconGrip, IconTag, IconX } from '../../../ui/primitives/Icons';
...
import StringInputAliasPicker from './StringInputAliasPicker';
      ...
      <div className="relative space-y-2" ref={wrapperRef}>
          ...
          <div className="stringinput-tokens">
            ...
            {tokens.map((t, idx) => {
              ...
              return (
                <div
                  key={`${t.token}-${t.index}`}
                  className={`ui-chip is-clickable token-chip token-chip-draggable ${isDragging ? 'is-dragging' : ''} ${isDropTarget ? 'is-drop-target' : ''}`}
                  title={`$${t.token}$`}
                  role="button"
                  tabIndex={0}
                  draggable
        ...
        {showPicker ? (
          <StringInputAliasPicker
            ...
            pickerSubcategory={pickerSubcategory}
            onSubcategoryChange={setPickerSubcategory}
            filteredAliases={filteredAliases}
            visibleAliases={visibleAliases}
            visibleCount={visibleCount}
            onShowMore={() => setVisibleCount((c) => Math.min(c + 30, filteredAliases.length))}
            listItemVisibilityStyles={listItemVisibilityStyles}
            onInsertAlias={handleInsertAlias}
          />
        ) : null}
      </div>
    );
