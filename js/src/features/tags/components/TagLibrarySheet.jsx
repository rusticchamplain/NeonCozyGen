...
import { IconCopy, IconDots, IconGrip, IconRefresh, IconTag, IconX } from '../../../ui/primitives/Icons';
...
export default function TagLibrarySheet({
  ...
  const handleTouchMove = useCallback((e) => {
    ...
    if (deltaX > 10 || deltaY > 10) {
      ...
      const chipElements = document.querySelectorAll('.tag-collection-chip');
                ...
                {collectedTags.length ? (
                  <div className="tag-collection-chips">
                    {collectedTags.map((tag, idx) => (
                      <span
                        key={`${tag}-${idx}`}
                        className="ui-chip is-accent is-clickable tag-collection-chip"
                      ...
                      >
                        <code className="ui-chip-code">{tag}</code>
                        <Button
                          size="mini"
                          variant="danger"
